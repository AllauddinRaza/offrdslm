    ...
    public class DailyRewards : DailyRewardsCore<DailyRewards>
    {
        ...
        public void ClaimPrize()
        {
            ...
            if (availableReward > 0)
            {
                ...
                if (GetLastRewardKey() == "Coins")
                {
                    PlayerPrefs.SetInt("Coins", PlayerPrefs.GetInt("Coins") + availableReward);
                    Debug.LogError(PlayerPrefs.GetInt("Coins"));
                }
                if (GetLastRewardKey() == "JOHN LI")
                {
                    PlayerPrefs.SetInt("Player4Buy", 1);
                }
                if (GetLastRewardKey() == "GHOST")
                {
                    PlayerPrefs.SetInt("Player1Buy", 1);
                }

                //GameObject.FindObjectOfType<MainMenu>().cashCounter.text = PlayerPrefs.GetInt("CashPref").ToString();
                //GameObject.FindObjectOfType<MainMenu>().CashCounterSettingPanel.text = PlayerPrefs.GetInt("CashPref").ToString();
                //Raza Mobile Notification
                //var notification = new AndroidNotification();
                //notification.Title = "Daily Reward";
                //notification.Text = "Come back and Claim Your Daily Reward";
                //notification.FireTime = System.DateTime.Now.AddMinutes(1440);

                //AndroidNotificationCenter.SendNotification(notification, "2222");
